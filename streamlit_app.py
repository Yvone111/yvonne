...
class GitHubExcelLoader:
    ...
    def __init__(self, github_repo=None, token=None):
        """
        初始化GitHub加载器
        github_repo: 格式为 "用户名/仓库名"，例如 "Yvonne/nutrition-data"
        token: GitHub个人访问令牌（可选，用于私有仓库）
        """
    ...
    def set_repo(self, github_repo, token=None):
        """设置GitHub仓库信息"""
        self.github_repo = github_repo
        if token:
            self.token = token
    ...
    def get_excel_files(self, path=""):
        """获取GitHub仓库中的Excel文件列表"""
        if not self.github_repo:
            return []
        
        try:
            url = f"{self.base_url}/{self.github_repo}/contents/{path}"
            ...
            st.error(f"获取GitHub文件列表失败: {str(e)}")
            return []
    ...
    def load_all_excel_files(self, path=""):
        """加载GitHub仓库中的所有Excel文件"""
        files = self.get_excel_files(path)
        monthly_data = {}
        
        for file_info in files:
            try:
                # 从文件名提取月份
                month_key = self.get_month_from_filename(file_info["name"])
                
                # 下载并读取文件
                df = self.download_excel_file(file_info["download_url"])
                if df.empty:
                    continue
                
                # 添加月份标识
                try:
                    if "利润模型评估报告_原始收益值_" in file_info["name"]:
                        date_str = file_info["name"].replace("利润模型评估报告_原始收益值_", "").replace(".xlsx", "")
                        file_date = datetime.strptime(date_str, "%Y%m")
                    else:
                ...
                df['月份'] = month_key
                df['日期'] = file_date
                df['数据来源'] = f"GitHub: {self.github_repo}"
                ...
                st.sidebar.success(f"✅ 已加载: {month_key} (来自GitHub)")
                
            except Exception as e:
                st.sidebar.error(f"❌ 处理文件 {file_info['name']} 失败: {str(e)}")
        
        return monthly_data


class NutritionAdviserDashboard:
    def __init__(self, data_folder=None, github_repo=None, github_token=None):
        """
        营养顾问绩效评估仪表板
        data_folder: 包含月度Excel报告文件的文件夹路径
        github_repo: GitHub仓库地址，格式为 "用户名/仓库名"
        ...
        if github_repo:
            self.github_loader = GitHubExcelLoader(github_repo, github_token)
    ...
    def load_all_data(self):
        ...
        # 加载GitHub文件
        if self.github_loader and self.github_loader.github_repo:
            self.github_data = self.github_loader.load_all_excel_files()
    ...
    def load_local_data(self):
        ...
        if not os.path.exists(self.data_folder):
            st.sidebar.info(f"本地文件夹不存在: {self.data_folder}")
            st.sidebar.info("将使用其他数据源")
            return
        ...
        if not excel_files:
            st.sidebar.info(f"在 {self.data_folder} 中没有找到本地Excel文件")
            return
    ...
    def merge_all_data(self):
        ...
        # 合并数据的优先级：上传文件 > GitHub > 本地文件
        all_sources = [
            ("uploaded", self.uploaded_data),
            ("github", self.github_data),
        ...
        for source_name, source_data in all_sources:
            for month_key, data_info in source_data.items():
                if month_key not in merged_months:
                    self.monthly_data[month_key] = data_info
                    merged_months.add(month_key)
        
        # 显示数据源统计
        local_count = len(self.local_data)
        github_count = len(self.github_data)
        uploaded_count = len(self.uploaded_data)
        
        if local_count > 0 or github_count > 0 or uploaded_count > 0:
            st.sidebar.info(f"📊 数据统计: 本地{local_count}个月, GitHub{github_count}个月, 上传{uploaded_count}个月")
    ...
    def create_overview_dashboard(self, selected_month):
        ...
        # 显示数据来源
        if selected_month in self.uploaded_data:
            data_source = "上传文件"
        elif selected_month in self.github_data:
            data_source = f"GitHub: {self.github_loader.github_repo if self.github_loader else '未知'}"
        elif selected_month in self.local_data:
            data_source = "本地文件"
        else:
            data_source = "未知"
    ...
    def create_adviser_type_chart(self, df, month):
        ...
        for label in sales_labels:
            if label in sales_distribution.columns:
                ...
                # sales_summary[f'{label}占比'] = sales_percentage[label].round(1)
        ...
        # 创建百分比堆叠条形图
        self.create_stacked_percentage_chart(sales_percentage, month)
    ...
    def create_stacked_bar_chart(self, sales_distribution, month):
        ...
        # 定义颜色
        colors = ['#8dd3c7', '#ffffb4', '#bebadb', '#fb8072']  # 蓝色、橙色、绿色、红色
        ...
        fig.update_layout(
            ...
            barmode='stack',
            height=500,  # 增加图表高度，为外部文本留出空间
            showlegend=True,
            margin=dict(l=50, r=50, t=80, b=50),  # 调整边距，为外部文本留出空间
        ...
        fig.update_yaxes(range=[0, max_value * 1.15])  # 增加15%的顶部空间
        
        st.plotly_chart(fig, use_container_width=True)
    
    def create_stacked_percentage_chart(self, sales_percentage, month):
        """使用go.Figure创建百分比堆叠条形图，并添加文本标注"""
        # 获取顾问类型和坎级标签
...
def main():
    ...
    if data_source == "GitHub仓库":
        ...
        with col1:
            github_repo_input = st.text_input(
                "GitHub仓库地址",
                value="Yvonne/nutrition-data",
                help="格式: 用户名/仓库名，例如: username/repo"
            )
        
        with col2:
            if st.button("验证", help="测试GitHub仓库连接"):
                if github_repo_input:
                    test_loader = GitHubExcelLoader(github_repo_input)
                    files = test_loader.get_excel_files()
                    if files:
                        st.success(f"✅ 找到 {len(files)} 个Excel文件")
                    else:
                        st.error("❌ 未找到Excel文件或仓库不可访问")
        
        github_token = st.sidebar.text_input(
            "GitHub Token (可选)",
            type="password",
            help="用于访问私有仓库的GitHub个人访问令牌"
        )
        
        if github_repo_input:
            github_repo = github_repo_input
            st.sidebar.info(f"📦 仓库: {github_repo}")
    
    elif data_source == "本地文件":
        st.sidebar.subheader("💾 本地文件")
        st.sidebar.info(f"将从以下路径读取: {data_folder}")
    ...
    # 处理上传的文件
    if uploaded_files and data_source == "文件上传":
        dashboard.load_uploaded_files(uploaded_files)
    
    # 刷新数据按钮
    if st.sidebar.button("🔄 刷新数据", help="重新加载所有数据源"):
    ...
    if available_months:
        ...
        ### 1. GitHub仓库模式（推荐）
        1. 在GitHub创建一个仓库（如: `Yvonne/nutrition-data`）
        2. 将Excel文件上传到仓库中
        3. 在侧边栏选择"GitHub仓库"模式
        4. 输入仓库地址（格式: 用户名/仓库名）
        5. 如果需要访问私有仓库，输入GitHub Token
        
        ### 2. 本地文件模式
        1. 将Excel文件放在指定文件夹中: `/Users/Yvonne/Desktop/伊利/人效分析/营养顾问分析报告`
        2. 在侧边栏选择"本地文件"模式
        ...
        1. **创建GitHub仓库**：用于存放Excel文件
        2. **文件命名**：建议使用 `利润模型评估报告_原始收益值_YYYYMM.xlsx` 格式
        3. **GitHub Token**（可选）：
           - 如果需要访问私有仓库，需要创建GitHub Token
        ...
        # GitHub使用指南
        with st.expander("📖 GitHub仓库设置详细指南", expanded=True):
            st.markdown("""
            ### 步骤1: 创建GitHub仓库
            1. 访问 [GitHub.com](https://github.com)
            2. 点击右上角"+"号 → "New repository"
            3. 输入仓库名称（如: `nutrition-data`）
            ...
            ### 步骤3: 获取仓库地址
            仓库地址格式为: `用户名/仓库名`
            例如: `Yvonne/nutrition-data`
            
            ### 步骤4: 在应用中配置
            1. 在侧边栏选择"GitHub仓库"模式
            2. 输入仓库地址
            3. 点击"验证"按钮测试连接
            4. 选择要分析的月份
            """)
